...
async function transaction_history (opts = {}) {
  ...
  for (const sub of subs) {
    const sub_drive = sdb.drive(sub.sid)
    const opts_file = await sub_drive.get('opts.json')
    const tx = opts_file.raw || {}

    const date = (tx.date || sub.date || 'Unknown').trim() // trim extra spaces
  ...
  for (const date in grouped) {
    ...
    for (const { tx, sub_drive } of grouped[date]) {
      const row = await transaction_row(tx, sub_drive)
